...
class TrigramSimpleTagger:
    ...
    def buildRulesMap(self):
        def wordMap(x):
            ...
            return (
                (
                    split[2],
                    split[3] if len(split) > 3 else "-",
                    split[4] if len(split) > 4 else "-",
                ),
                float(split[0]),
            )
    ...
    def tagSentence(self, words):
        ...
        current_tuple = reduce(
            lambda x, y: x if p[len(p) - 1][x][0] > p[len(p) - 1][y][0] else y,
            p[len(p) - 1],
        )
...
fin = "gene.count.out"  # "data/ner_train_counts"
raw = "gene.test"  # data/ner_dev.dat"

tagger = TrigramSimpleTagger(fin, word_classifier.SimpleClassifier())
sentences = open(raw).read().split("\n\n")
writer = open("out.txt", "w")
for sentence in sentences:
    words = sentence.split("\n")
    # rint words
    # empty unicode in tag for empty line.
    if len(words) > 1:
